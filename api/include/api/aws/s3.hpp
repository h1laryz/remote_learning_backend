...
public:
    ...
    bool createBucket( const Aws::String& bucketName ) const
    {
        ...
        {
            std::cout << "Created bucket " << bucketName << " in the specified AWS Region."
                      << std::endl;
        }
...
public:
    ...
    [[nodiscard]] std::optional< std::string > add( std::string_view subject,
                                                    ...
                                                    std::string_view homework_content ) const
    {
        ...
        std::string key =
            std::string( group ) + "/" + std::string( subject ) + "/" + std::string( file_name );
        ...
        *data << homework_content;
        ...
        if ( outcome.IsSuccess() )
        {
            std::cout << "Uploaded homework file '" << file_name << "' successfully to S3."
                      << std::endl;
            return key;
        }

        auto& err = outcome.GetError();
        std::cerr << "Error uploading homework file '" << file_name
                  << "' to S3: " << err.GetExceptionName() << ": " << err.GetMessage() << std::endl;
        return std::nullopt;
    }
        ...
        if ( outcome.IsSuccess() )
        {
            ...
            for ( const auto& object : outcome.GetResult().GetContents() )
            {
                ...
                else
                {
                    ...
                    std::cerr << "Error getting homework file '" << object.GetKey().c_str()
